...
class Page extends _events.default {
  ...
  async init({
    ...
    let [, {
      frameTree
    }] = await Promise.all([this.send('Page.enable'), this.send('Page.getFrameTree')]);

    _classPrivateFieldSet(this, _frameId, frameTree.frame.id);
  ...
  async goto(url, {
    ...
    try {
      ...
      await Promise.all([navigate, (0, _waitFor.default)(() => {
    ...
    await this.network.idle(); // wait for any specified timeout
