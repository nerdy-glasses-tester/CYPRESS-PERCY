...
class PercyDiscoverer {
  ...
  async page({
    ...
    page.network.authorization = authorization; // set page options

    await Promise.all([page.send('Network.setCacheDisabled', {
      cacheDisabled
    }), page.send('Network.setExtraHTTPHeaders', {
      headers: requestHeaders
    }), page.send('Security.setIgnoreCertificateErrors', {
      ignore: ignoreHTTPSErrors
    }), page.send('Emulation.setScriptExecutionDisabled', {
  ...
  gatherResources({
    ...
    return this.queue.push(async () => {
      ...
      try {
        ...
        page.network.onrequest = this._handleRequest({
          meta,
          rootUrl,
          rootDom
        });
        page.network.onrequestfinished = this._handleRequestFinished({
          meta,
          rootUrl,
          onDiscovery
        });
        page.network.onrequestfailed = this._handleRequestFailed({
          meta
        });
    ...
    return async request => {
      ...
      try {
        if (url === rootUrl) {
          ...
          (0, _assert.default)(allowedHostnames.some(h => (0, _url.domainMatch)(h, url)), 'is remote', meta);
